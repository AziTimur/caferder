...
public class zakazActivity extends AppCompatActivity {
    ...
    private StringBuilder builderAdditioms;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (intent.hasExtra("name")&& intent.hasExtra("password"))
        {
            ...
            name=getString(R.string.defauld_name);
            password=getString(R.string.defauld_password);
        }
        ...
        builderAdditioms=new StringBuilder();
    ...
    public void onClickSendOrder(View view) {
        builderAdditioms.setLength(0);
        if (checkBoxMilk.isChecked()){
            builderAdditioms.append(getString(R.string.milk)).append(" ");
        }
        if (checkBoxLemon.isChecked()&& drink.equals(R.string.tea)){
            builderAdditioms.append(getString(R.string.lemon)).append(" ");
        }
        if (checkBoxSugar.isChecked()){
            builderAdditioms.append(getString(R.string.sugar)).append(" ");
        }
        ...
        String order= String.format(getString(R.string.order2),name,password,drink,optionOfDrink);
        String additions;
        if (builderAdditioms.length()>0){
            additions="\n"+ getString(R.string.need_additions)+builderAdditioms.toString();
        }else {
            additions="";
        }
        String fullOrder=order+additions;
        Intent intent=new Intent(this,OrderDetailActivity2.class);
